...
public class BR {
	...
	// CNPJ
	private static final int[] WEIGHT_TFN = { 6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4,
			3, 2 };
	...
	private static int sum(int[] weight, char[] numbers, int length) {
		...
		return (sum(weight, numbers, nIdx)
				+ Character.getNumericValue(numbers[nIdx]) * weight[wIdx]);
	}
